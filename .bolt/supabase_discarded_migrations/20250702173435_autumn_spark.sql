/*
  # Fix RLS Policies for Organization Dashboard

  1. Helper Functions
    - Drop and recreate all helper functions with proper signatures
  ...
  2. RLS Policies
    ...
    - Recreate comprehensive policies for topics, events, organizations, and memberships
    - Ensure proper access control based on roles and organization membership

  3. Security
    - Super admins have full access to everything
    - Organization members can access their organization's content
    - Proper role-based permissions (admin, editor, viewer)
*/

-- Drop ALL existing RLS policies for ALL tables to avoid conflicts
...
-- Drop all existing helper functions with CASCADE to handle dependencies
...
-- Events inherit permissions from their parent topic
...
-- Create RLS policies for ORGANIZATION_MEMBERSHIPS table
